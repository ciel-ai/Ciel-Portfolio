...
export default function ImagePlaceholder({ src, label, title, accent, featured = false }: ImagePlaceholderProps) {
  return (
    <div
      className={`relative overflow-hidden border-[4px] border-ciel-ink bg-ciel-primary ${
        featured ? 'aspect-[16/10]' : 'aspect-video'
      }`}
      style={{ boxShadow: `4px 4px 0 ${accent}` }}
    >
      {src ? (
        <img className="h-full w-full object-cover" src={src} alt={`${title} ${label}`} loading="lazy" />
      ) : (
